...
async def get_list_jobs(content, stack: str):
    vacancies = content.find_all(
        'div', attrs={'class': 'flex-auto min-width-0 p-2 pr-3 pr-md-2'})
    vacancies.reverse()
    data = []
    stack_label = stack
    for vacancie in vacancies:
        ...
        if month == datetime.now().month and int(date_posted[1]) == datetime.now().day:
            title = vacancie.find('a', attrs={
                                  'class': 'Link--primary v-align-middle no-underline h4 js-navigation-open markdown-title'})
            requirements = vacancie.find(
                'span', attrs={'class': 'lh-default d-block d-md-inline'})
            ...
            data.append({'title': title.text, 'requirements': requirements,
                        'link': link, 'stack': stack_label})
